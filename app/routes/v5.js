...
router.post("/"+v+"/san/:ref/support/edit/:thiscat/:thiscount", function (req, res) {
  ...
  //make textarea content safe
  let editNeedHTML = req.session.data["san-"+v+"-"+ref+"-support-desc"].replace(/(?:\r\n|\r|\n)/g, '<br>');
  //create new support strat object
  let newNeedEntry = {
    needsSupportCategory: thiscat,
    needsSupportDescription: editNeedHTML,
    needsSupportDate: getFormattedDate(),
    needsSupportAuthor: "W. Knight",
    needsSupportLastAction: "Edited"
  }
  
  let thisprisoner = req.session.data[v+'prisoners'].find(p => p.prisonerNumber === ref);
  //empty existing support startegy item
  thisprisoner.needsSupport.splice(thiscount-1, 1);
  //push new item
  thisprisoner.needsSupport.push(newNeedEntry);
  delete req.session.data["san-"+v+"-"+ref+"-support-desc"];
...
router.post("/"+v+"/san/:ref/support/archive/:thiscat/:thiscount", function (req, res) {
  ...
  //make textarea content safe
  let editNeedHTML = req.session.data["san-"+v+"-"+ref+"-support-desc"].replace(/(?:\r\n|\r|\n)/g, '<br>');
  let archiveNeedHTML = req.session.data["san-"+v+"-"+ref+"-support-archive"].replace(/(?:\r\n|\r|\n)/g, '<br>');
  //create new support strat object
  let newNeedEntry = {
    needsSupportCategory: thiscat,
    needsSupportDescription: editNeedHTML,
    needsSupportDate: getFormattedDate(),
    needsSupportAuthor: "W. Knight",
    needsSupportLastAction: "Moved to History",
    needsSupportArchiveReason: archiveNeedHTML
  }
...
router.post("/"+v+"/san/:ref/challenges/edit/:thiscat/:thiscount", function (req, res) {
  ...
  //make textarea content safe
  let editChallengeHTML = req.session.data["san-"+v+"-"+ref+"-support-desc"].replace(/(?:\r\n|\r|\n)/g, '<br>');
  //create new support strat object
  let newChallengeEntry = {
    needsChallengeCategory: thiscat,
    needsChallengeDescription: editChallengeHTML,
    needsChallengeDate: getFormattedDate(),
    needsChallengeAuthor: "W. Knight",
    needsChallengeLastAction: "Edited"
  }
  
  let thisprisoner = req.session.data[v+'prisoners'].find(p => p.prisonerNumber === ref);
  //empty existing support startegy item
  thisprisoner.needsChallenges.splice(thiscount-1, 1);
  //push new item
  thisprisoner.needsChallenges.push(newChallengeEntry);
  delete req.session.data["san-"+v+"-"+ref+"-support-desc"];

  res.redirect("/"+v+"/san/"+ref+"/profile/challenges-strategies/");
...
router.post("/"+v+"/san/:ref/challenges/archive/:thiscat/:thiscount", function (req, res) {
  ...
  //make textarea content safe
  let editChallengeHTML = req.session.data["san-"+v+"-"+ref+"-support-desc"].replace(/(?:\r\n|\r|\n)/g, '<br>');
  let archiveChallengeHTML = req.session.data["san-"+v+"-"+ref+"-support-archive"].replace(/(?:\r\n|\r|\n)/g, '<br>');
  //create new support strat object
  let newNeedEntry = {
    needsSupportCategory: thiscat,
    needsSupportDescription: editChallengeHTML,
    needsSupportDate: getFormattedDate(),
    needsSupportAuthor: "W. Knight",
    needsSupportLastAction: "Moved to History",
    needsSupportArchiveReason: archiveChallengeHTML
  }
